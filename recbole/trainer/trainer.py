...
class Trainer(AbstractTrainer):
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if stop_flag:
                    if self.config['train_stage'] == 'alltrain':
                        if self.model.state == 'disc':
                            stop_output = 'Finished training 1st part, best eval result in epoch %d' % \
                            ...
                            self.model.load_state_dict(checkpoint['state_dict'])
                            self.model.load_other_parameter(checkpoint.get('other_parameter'))
                            message_output = 'Loading best 1st model structure and parameters from {}'.format(checkpoint_file)
                            self.logger.info(message_output)

                            self.model.state = 'sr'
                            self.cur_step = 0
                            self.model.discrim_loss_weight = 0
                            self.model.generate_loss_weight = 0
                        else:
                            stop_output = 'Finished training 2st part, best eval result in epoch %d' % \
                                          (epoch_idx - self.cur_step * self.eval_step)
                            if verbose:
                                self.logger.info(stop_output)
                            break
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        ...
        if hasattr(self.model, 'recall_n') and self.model.recall_n:
            r = self.model.recall / self.model.recall_n
            print('------Generate back ratio: %.3f' % (r * 100) + '%' + ', stage: ' + self.model.state + '------')
            self.model.recall_n = 0
            self.model.recall = 0
        if hasattr(self.model, 'auto_weight'):
            print('------%f, %f, %f, %f------' % (self.model.encode_loss_weight, self.model.con_loss_weight, self.model.discrim_loss_weight, self.model.generate_loss_weight))
