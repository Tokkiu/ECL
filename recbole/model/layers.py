...
class NOVAMultiHeadAttention(nn.Module):
    """
    Multi-head non-invasive Self-attention layers, a attention score dropout layer is introduced.
    Args:
        input_tensor (torch.Tensor): the input of the non-invasive multi-head self-attention layer
        attention_mask (torch.Tensor): the attention mask for input tensor
    Returns:
        hidden_states (torch.Tensor): the output of the multi-head self-attention layer
    """
    ...
    def forward(self, input_tensor, attribute_tensor, attention_mask):
        item_value_layer = self.transpose_for_scores(self.value(input_tensor))

        attribute_query_layer = self.transpose_for_scores(self.query(attribute_tensor))
        attribute_key_layer = self.transpose_for_scores(self.key(attribute_tensor))

        # Take the dot product between "query" and "key" to get the raw attention scores.
        attribute_attention_scores = torch.matmul(attribute_query_layer, attribute_key_layer.transpose(-1, -2))

        attention_scores = attribute_attention_scores
...
class NOVATransformerLayer(nn.Module):
    """
    One transformer layer consists of a non-invasive multi-head self-attention layer and a point-wise feed-forward layer.
    Args:
        hidden_states (torch.Tensor): the input of the non-invasive multi-head self-attention sublayer
    ...
    def forward(self, hidden_states, attribute_embed, attention_mask):
        attention_output = self.multi_head_attention(hidden_states, attribute_embed, attention_mask)
        feedforward_output = self.feed_forward(attention_output)
        return feedforward_output


class FilterLayer(nn.Module):
    def __init__(self,  max_seq_length, hidden_size, hidden_dropout_prob, layer_norm_eps):
        super(FilterLayer, self).__init__()

        self.complex_weight = nn.Parameter(torch.randn(1, max_seq_length//2 + 1, hidden_size, 2, dtype=torch.float32) * 0.02) #todo：why？
        self.out_dropout = nn.Dropout(hidden_dropout_prob)
        self.LayerNorm = nn.LayerNorm(hidden_size, eps=layer_norm_eps)
...
class FMLPLayer(nn.Module):
    ...
    def __init__(
        self, max_seq_length, hidden_size, intermediate_size, hidden_dropout_prob, hidden_act,
        layer_norm_eps
    ):
        super(FMLPLayer, self).__init__()
        self.filter_layer = FilterLayer(max_seq_length, hidden_size, hidden_dropout_prob, layer_norm_eps)
        self.feed_forward = FeedForward(hidden_size, intermediate_size, hidden_dropout_prob, hidden_act, layer_norm_eps)
...
class NOVATransformerEncoder(nn.Module):
    r""" One non-invasive TransformerEncoder consists of several TransformerLayers.
    ...
    def forward(self, hidden_states, attribute_embed, attention_mask, output_all_encoded_layers=True):
        """
        Args:
        ...
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, attribute_embed, attention_mask)
...
class FMLPEncoder(nn.Module):
    ...
    def __init__(
        ...
        layer = FMLPLayer(
            max_seq_length, hidden_size, inner_size, hidden_dropout_prob, hidden_act,
            layer_norm_eps
        )
        self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(n_layers)])
