...
class SequentialDataset(Dataset):
    ...
    def data_augmentation(self):
        ...
        if self.config['disable_aug'] == 'y':
            ...
            for k,v in arr_d.items():
                for j in range(2, -1, -1):
                    uid_list.append(k)

                    if len(v) - 1 - j > max_item_list_len:
                        item_list_index.append(slice(v[-1-max_item_list_len-j], v[-1-j]))
                        target_index.append(v[-1-j])
                        item_list_length.append(max_item_list_len)
                    else:
                        item_list_index.append(slice(v[0], v[-1-j]))
                        target_index.append(v[-1-j])
                        item_list_length.append(len(v)-1-j)

        # half aug
        elif self.config['disable_aug'] == 'h':
            arr_d = {}
            for i, uid in enumerate(self.inter_feat[self.uid_field].numpy()):
                if uid not in arr_d:
                    arr_d[uid] = [i]
            ...
            for k, v in arr_d.items():
                ...
                last = v[-3]
                start = v[0]
                if last - start > max_item_list_len:
                    last = last - max_item_list_len

                    while last - start > 10:
                        nstart = max(last-max_item_list_len, start)
                        item_list_index.append(slice(nstart, last))
                        target_index.append(last)
                        item_list_length.append(last - nstart)
                        uid_list.append(k)
        ...
        # half aug, but more
        elif self.config['disable_aug'] == 'hm':
            half_item_list_len = int(0.5 * max_item_list_len)
            arr_d = {}
            for i, uid in enumerate(self.inter_feat[self.uid_field].numpy()):
                if uid not in arr_d:
                    arr_d[uid] = [i]
            ...
            for k, v in arr_d.items():
                ...
                last = v[-3]
                start = v[0]
                if last - start > max_item_list_len + 10:
                    last = last - half_item_list_len
            ...
            for i, uid in enumerate(self.inter_feat[self.uid_field].numpy()):
                if last_uid != uid:
                    ...
                    # @todo for ECL only,
                    # @todo it seems the operation will improve beauty and toys a lot
                    # if i - seq_start < 50:
                    #     continue
